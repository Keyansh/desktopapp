...
class Cms extends Cms_Controller
{
    ...
    public function projectData()
    {
        ...
        $html .=  '<td><input type="text" class="amount" name="amount[]" value="' . $data["mrprs"] . '" placeholder="Amount" readonly></td>';
    ...
    public function insertHold()
    {
        ...
        $data = array();
        $data['order_data'] = json_encode(array('hold_data' => $array_new));
        $data['added_on'] = time();
        $data['order_number'] = 'order-'.date(Ymd).'-'.time();
        $this->db->insert('order_hold', $data);
        return;
    }
}
