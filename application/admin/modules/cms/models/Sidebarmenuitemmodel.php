...
class Sidebarmenuitemmodel extends CI_Model {
    ...
    function details($pid) {
        $this->db->from('sidebar_menu_item');
    ...
    function getAll($menu_id) {
        ...
        $rs = $this->db->get('sidebar_menu_item');
    ...
    function indentedLists($menu_id, &$output = array()) {
        ...
        $query = $this->db->get('sidebar_menu_item');
    ...
    function insertRecord($menu_detail) {
        ...
        $this->db->insert('sidebar_menu_item', $data);
        return;
    }
    ...
    function menuTree($parent, $mid, $output = '') {
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result_array() as $row) {
                ...
                if ($row['url'] != '') {
                    $edit_href = 'cms/sidebar_menu_item/editurl/' . $row['menu_item_id'];
                } elseif ($row['is_placeholder'] == 1) {
                    $edit_href = 'cms/sidebar_menu_item/edit_placeholder/' . $row['menu_item_id'];
                } else {
                    $edit_href = 'cms/sidebar_menu_item/edit/' . $row['menu_item_id'];
                }
    ...
    function menuItemTree($parent, $mid, $output = '') {
        ...
        if ($query->num_rows() > 0) {
            if ($parent == 0) {
                $output .= '<ul id="sidebar-menutree">' . "\r\n";
            } else {
                $output .= "<ul>\r\n";
            }
            foreach ($query->result_array() as $row) {
                $edit_href = '';
                $del_href = 'cms/sidebar_menu_item/delete/' . $row['menu_item_id'];
                ...
                if ($row['category_id'] != '') {
                    ...
                    $edit_href = 'cms/sidebar_menu_item/update_url/' . $row['menu_item_id'];
                }
