...
class Attributesmodel extends CI_Model
{
    ...
    {
        if ($this->input->post('for_swatches', true) == 1) {
            self::removeSwatchesDisplay();
        }
        $data = array();
        ...
        $data['front_view'] = $this->input->post('front_view', true);
        $data['for_layered'] = $this->input->post('for_layered', true);
        $data['for_swatches'] = $this->input->post('for_swatches', true);
        //$data['defaultShow'] = $this->input->post('default', true);
        $this->db->insert('attribute', $data);
    }

    private function removeSwatchesDisplay()
    {
        $this->db->set('for_swatches', 0)->update('attribute');
    }
