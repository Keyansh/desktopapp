...
class Categorymodel extends CI_Model {
    ...
    function insertRecord() {
        ...
        $data['category_template'] = $this->input->post('category_template', true);
    ...
    function updateRecord($category) {
        ...
        if($update) {
            $parent_ids = categoryParentTree($category['id'], 0);
            $this->db->where('child_id',$category['id'])
            ->delete('cat_parent_child');
            ;
            foreach($parent_ids as $level => $parent_id):
                ...
                $this->db->insert('cat_parent_child',$tmp);
            endforeach;
        }
    }
    ...
    function deleteRecord($current_category) {
        ...
        $this->db->where('cid', $current_category['id']);
        $query = $this->db->get('cat_prod');
...
//                <a href=\"" . $attribute_href . "\">Assign Attribute</a> |
    ...
    //list all category
    function getSearchedCategory($parent, $keyword) {
        $this->db->where('active', 1);
//        $this->db->where('parent_id', $parent);
        $this->db->order_by('sort_order', 'ASC');
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result_array() as $row) {
                ...
                $output .= '<li id="page_' . $row['id'] . '">'
                        . '<div class="page_item">'
                        . '<div class="page_item_name ">'
                        . '<a href="' . $edit_href . '">' . $row['name'] . "</a>"
                        . "</div>"
                        . "<div class=\"menu_item_options\">"
                        . "<a href='catalognew/product/catproducts/" . $row['id'] . "' title='Manage Products'>Manage Products</a> |"
                        . "<a href=\"" . $link . "\" onclick=\"return confirm('Are you sure you want to " . $link_type . " this Category ?');\" title='Enable/Disable'>" . $link_type . "</a> | "
                        . "<a href=\"" . $edit_href . "\" title=''Edit><i class='glyph-icon icon-linecons-pencil'></i></a> | "
                        . "<a href=\"" . $del_href . "\" onclick=\"return confirm('Are you sure you want to Delete this Link ?');\" title='Delete'><i class='glyph-icon icon-linecons-trash red-color'></i></a>"
                        . "</div>"
    ...
    function find_parent_id($cat) {
        $result = $this->db->select('name,parent_id,id')
                        ->from('category')
    ...
    function check_child_id($id) {
        return $this->db->select('child_id')
                        ->from('cat_parent_child')
                        ->where('child_id', $id)
                        ->get()->row_array();
    }
