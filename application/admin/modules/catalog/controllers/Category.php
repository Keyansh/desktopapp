...
class Category extends Admin_Controller
{
    ...
    function categoryFilter()
    {
        ...
        if ($keyword == '') {
            $searchedCats = $this->Categorymodel->categoriesTree(0);
        } else {
            $searchedCats = $this->Categorymodel->getSearchedCategory(0, $keyword);
        }
        if ($searchedCats) {
            echo $searchedCats;
        } else {
            echo '<p class="no-records">No category found matching <span>' . $keyword . '</span> keyword!</p>';
        }
    }

    function remove_cat_img($cat_id)
    {
        $category = $this->Categorymodel->getdetails($cat_id);
        if (!$category) {
            $this->utility->show404();
            return;
        }
        $this->Categorymodel->remove_img($category);
        $this->session->set_flashdata('SUCCESS', 'category_updated');
        redirect('catalog/category/edit/' . $cat_id);
        exit();
    }
}
