                ...
                if ( $p_NamedFields ) {
                    ...
                    foreach ( $this->fields as $id => $field ) {
                        if ( isset($row[$id]) ) {
                            // $items[$field] = $row[$id];
                            $to_encoding   = 'UTF-8';
                            $from_encoding = mb_detect_encoding($row[$id]);
                            $items[$field] = mb_convert_encoding($row[$id], $to_encoding, $from_encoding);
//                            $items[$field] = iconv($to_encoding, "$to_encoding//IGNORE",$row[$id]);
                            $items[$field] = trim($items[$field]);
                        }
                    }
                    $content[] = $items;
                } else {
